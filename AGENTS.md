 ...
 * **PACOTE ENG #11 Assiduidade + faltaDesconto gate (2026-09-06):** faltas/descontos só com `em_custodia`; RPC `log_falta` (revoga INSERT directo); trigger `handle_falta_desconto` zera desconto sem pagamento; RPC `admin_liquidate_payment` (repasse = payout − Σ faltas mês); colunas `desconto_faltas_kz` / `valor_repasse_kz` / `liquidado_em`; utils `allowsAssiduidadeFaltas` / `computeRepasseLiquidoKz`; gates UI em `MyAgreements` + `AbsenceTracker`; admin liquidação em `/admin/pagamentos`. Migração `20260907020000_pacote_eng11_assiduidade_faltadesconto_gate.sql` aplicada remoto. Spec: `.specs/features/pacote-eng-11-assiduidade/quick.md`.
...
**Próximo:** Política ida/regresso (desconto). Opcional: TTL reservas; polish admin Critiquito; idempotência DB em `reject_agreement_adenda`. **Fora do MVP:** zonas/polígonos/raio residencial; adenda bilateral completa (hoje motorista inicia). Commits só se o utilizador pedir.
