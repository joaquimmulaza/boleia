...
-- Source: production migration history sync — 20260906004034 rpc_idempotency_wave4_accept_proposal
-- Do not rename; Supabase Preview CI requires exact version match.

-- Wave 4: accept_proposal + p_idempotency_key
-- Bootstrap gap (Wave 3): rpc_idempotency + leave_passenger/cancel_proposal idempotency
-- exist on production but were applied outside schema_migrations (no tracked version).
-- Prepended here so preview/fresh databases succeed before Wave 4 RPC updates.
-- Production already has these objects; IF NOT EXISTS / CREATE OR REPLACE are safe.
