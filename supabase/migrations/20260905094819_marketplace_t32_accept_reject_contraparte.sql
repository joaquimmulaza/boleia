...
-- Source: production migration history sync — 20260905094819 marketplace_t32_accept_reject_contraparte
-- Do not rename; Supabase Preview CI requires exact version match.

-- T32: aceite/rejeição só pela contraparte (created_by bloqueado)
...
CREATE OR REPLACE FUNCTION public.reject_proposal(p_proposta_id uuid)
...
BEGIN
  ...
  SELECT * INTO v_procura FROM public.procuras WHERE id = v_prop.procura_id;
  IF v_uid IS DISTINCT FROM v_oferta.driver_id AND v_uid IS DISTINCT FROM v_procura.owner_id THEN
    RAISE EXCEPTION 'Sem permissão para rejeitar esta proposta.';
  END IF;

  UPDATE public.propostas
  SET estado = 'rejeitada', updated_at = now()
  WHERE id = v_prop.id
...
GRANT EXECUTE ON FUNCTION public.reject_proposal(uuid) TO service_role;

-- RLS: envolvidos podem actualizar, excepto o criador (defesa em profundidade)
...
CREATE POLICY propostas_update_envolvidos ON public.propostas
  FOR UPDATE
  USING (
    auth.uid() IS DISTINCT FROM created_by
    AND (
      auth.uid() = (SELECT o.driver_id FROM ofertas_capacidade o WHERE o.id = propostas.oferta_id)
      OR auth.uid() = (SELECT p.owner_id FROM procuras p WHERE p.id = propostas.procura_id)
    )
  )
  WITH CHECK (
    auth.uid() IS DISTINCT FROM created_by
