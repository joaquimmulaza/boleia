...
-- Source: production migration history sync — 20260904135819 marketplace_t7b_fix_acordo_notifications_timing
-- Do not rename; Supabase Preview CI requires exact version match.

-- Fix: no INSERT do acordo, acordos_passageiros ainda não existem.
-- Trigger notifica só o motorista; accept_proposal notifica os N passageiros após insert.
...
BEGIN
  ...
  -- Notificar passageiros após insert das linhas N
  INSERT INTO public.notificacoes (user_id, mensagem, tipo, metadata)
  SELECT
    ap.passenger_id,
